        ...
        <div class="mt-[4.375rem] mb-[4.375rem] w-[80%] grid grid-cols-4 gap-6">
            <div>
                ...
                <div class="space-y-6 ml-10 my-4 flex flex-col gap-[1px]">
                    ...
                    <a href="{{ route('profile.Order_completed') }}" class="text-orange-500">
            ...
            <div class="bg-white shadow-md col-span-3 rounded-xl p-10 space-y-10">
                <h3 class="text-lg font-semibold">สำเร็จการสั่งซื้อ</h3>
                @if ($orders->isEmpty())
                    <div class="text-center ">
                        <p class="text-lg text-gray-600">ยังไม่มีคำสั่งสินค้าที่ชำระแล้ว</p>
                    </div>
                @else
                    <div class="w-full">
                        <div class="">
                            <input type="text" class="w-full p-3 rounded-lg border border-gray-300"
                                placeholder="ค้นหาโดยใช้ชื่อสินค้า" />
                        </div>
                        <div class="px-6 mt-2">
                                ...
                                @if ($order->product && $order->status === 'completed')
                                    <a href="{{ route('profile.orders.seemore', ['allOrderId' => $order->all_order_id]) }}">
                                        <div
                                            ...
                                            <div class="grid grid-cols-2 ">
                                                <div class="flex flex-row gap-4">
                                                    <div class="flex flex-col items-center">
                                                        <div>
                                                            <img src="{{ $order->product->product_img }}"
                                                ...
                                                <div class="flex flex-col gap-2 items-end justify-end text-right">
                                                    <div class="flex flex-col gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p class="text-sm text-gray-600">{{ $order->quantity }} รายการ
                                                            </p>
                                                        </div>
                                                        <div>
                                                            <p class="text-sm text-gray-600">ราคารวมค่าส่งและภาษี : ฿
                                                                {{ $order->total_price }}</p>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div class="flex items-center justify-center">
                                                <a href="{{ route('profile.orders.seemore', ['allOrderId' => $order->all_order_id]) }}"
                                                    class="hover:text-orange-500 hover:font-bold">
                                                    ดูเพิ่มเติม
                                                </a>
                                            </div>
                                        </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            searchInput.addEventListener('input', function() {
                ...
                orderItems.forEach(item => {
                    const productName = item.querySelector('h1.text-lg.font-semibold').textContent
                        .toLowerCase();
